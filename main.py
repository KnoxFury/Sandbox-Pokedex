...
def find(query):
    q = query.strip().lower()
    row = df[df["pokedex_number"] == int(q)] if q.isdigit() else df[df["name"].str.lower() == q]
...
# ── Search bars ──────────────────────────────────────────────────────────────
e1 = tk.Entry(root, width=20); e1.grid(row=0, column=0, padx=4, pady=4)
e2 = tk.Entry(root, width=20); # revealed by Compare button
...
def search(idx, entry):
    p = find(entry.get())
    if p is None: messagebox.showwarning("Not found", f"'{entry.get()}' not found."); return
    pokemon[idx] = p
    show_stats(idx, p)
    if pokemon[0] is not None: draw_radar()
...
# ── Radar chart ──────────────────────────────────────────────────────────────
chart_frame = tk.Frame(root); chart_frame.grid(row=3, column=0, columnspan=3)
...
# ── Battle log ───────────────────────────────────────────────────────────────
def run_battle():
    if any(p is None for p in pokemon): messagebox.showwarning("Missing", "Search both Pokemon first."); return
    log = simulate_battle(pokemon[0].to_dict(), pokemon[1].to_dict())
    log_box.config(state="normal"); log_box.delete("1.0", "end")
    log_box.insert("end", log); log_box.config(state="disabled")
